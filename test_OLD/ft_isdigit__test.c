...
int	test0()
{
	...
	print_test_result(ok, "Digit character parameters must return true.");
	return (ok);
}
...
int	test1()
{
	...
	print_test_result(ok, "Non-digit character parameters must return false.");
	return (ok);
}

int	main(void)
{
	print_test_title("int ft_isdigit(int c)");
	test0();
	test1();
	return (0);
}
